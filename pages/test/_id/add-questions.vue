      ...
      <v-col cols="12" md="6">
        <v-card>
          <v-card-text class="text-center">
            ...
            <div class="text-h6 mt-4">الاختبار غير موجود</div>
            <div class="text-caption mt-2">الاختبار الذي تبحث عنه غير موجود أو ليس لديك صلاحية لتعديله.</div>
            <v-btn color="primary" class="mt-4" to="/create-test">إنشاء اختبار جديد</v-btn>
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    handleApiError(error, action = 'تنفيذ العملية') {
      try {
        let errorMessage = `فشل ${action}`
        
        if (error && error.response && error.response.data) {
          const errorData = error.response.data
          
          // Case 1: errors is a string (custom exception like NotEnoughQuestions)
          if (errorData.errors !== undefined) {
            if (typeof errorData.errors === 'string') {
              errorMessage = errorData.errors
            } 
            // Case 2: errors is an object with validation errors
            else if (typeof errorData.errors === 'object' && !Array.isArray(errorData.errors)) {
              // Check if it's validation errors (object with arrays as values)
              ...
              if (isValidationErrors) {
                ...
                return // Exit early after showing validation errors
              } else {
                // errors object but not validation format, try to extract message
                errorMessage = errorData.errors.message || JSON.stringify(errorData.errors)
              }
            }
          }
          ...
          // Case 4: error field
            ...
            errorMessage = typeof errorData.error === 'string' ? errorData.error : 'حدث خطأ'
          }
        ...
        // Prevent error handling from breaking the page - this should never happen but just in case
        console.error('Critical error in handleApiError:', err)
        try {
          this.$toast.error(`فشل ${action}. يرجى المحاولة مرة أخرى.`)
        } catch (toastErr) {
          // Even toast failed, just log it
          console.error('Toast also failed:', toastErr)
        }
      }
    ...
    async loadData() {
      ...
      try {
        ...
        if (testRes.data && testRes.data.length > 0 && testRes.data[0]) {
          ...
          this.$toast.error('الاختبار غير موجود. تأكد من وجوده وأنه اختبار مسودة.')
        }
        ...
        this.$toast.error('فشل تحميل الاختبار: ' + (error.response?.data?.message || error.message || 'خطأ غير معروف'))
      } finally {
        this.loading = false
      }
    },
    ...
    async createQuestion() {
      if (!this.questionForm.question || !this.questionForm.question.trim()) {
        this.$toast.error('نص السؤال مطلوب')
        return
      }
      ...
      try {
        ...
        this.handleApiError(error, 'إنشاء السؤال')
      } finally {
        this.loading = false
      }
    },
    async updateQuestion() {
      ...
      try {
        ...
        this.handleApiError(error, 'تحديث السؤال')
      } finally {
        this.loading = false
      }
    },
    ...
    async updateTest() {
      ...
      try {
        ...
        await this.loadData() // Refresh test data
        this.testForm = false
        this.$toast.success('تم تحديث الاختبار بنجاح!')
      } catch (error) {
        this.handleApiError(error, 'تحديث الاختبار')
      } finally {
        this.loading = false
      }
    },
    async publishTest() {
      ...
      try {
        ...
        this.handleApiError(error, 'نشر الاختبار')
      } finally {
        this.loading = false
      }
    }
